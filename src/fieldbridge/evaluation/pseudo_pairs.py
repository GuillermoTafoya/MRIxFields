...
def evaluate_pseudo_pairs(
    ...
    with torch.no_grad():
        for raw_batch in loader:
            ...
            wrong_domains = _wrong_target_domains(batch.target_domain, cfg.target_fields)
            ...
            x_wrong = from_model_range(wrong_prediction, cfg.model_range).clamp(0.0, 1.0)
            ...
            for index, target_domain in enumerate(batch.target_domain):
                ...
                rows.append(
                    {
                        "target_field": f"{target_domain.field_strength_t:g}T",
                        "degraded": _compute_metrics(
                            x_low[index : index + 1],
                            target,
                            mask,
                            lpips_net=lpips_net,
                        ),
                        "predicted": _compute_metrics(
                            x_pred[index : index + 1],
                            target,
                            mask,
                            lpips_net=lpips_net,
                        ),
                        "wrong_conditioned": _compute_metrics(
                            x_wrong[index : index + 1],
                            target,
                            mask,
                            lpips_net=lpips_net,
                        ),
    ...
    conditioning_delta = _metric_delta(aggregate["wrong_conditioned"], aggregate["predicted"])
    permuted_delta = _metric_delta(aggregate["permuted_conditioned"], aggregate["predicted"])
    return {
        ...
        "target_conditioning_audit": {
            "correct_vs_wrong_improvement": conditioning_delta,
            "correct_vs_permuted_improvement": permuted_delta,
            "correct_conditioning_improves_nrmse": conditioning_delta.get("nrmse", 0.0) > 0.0,
            "correct_conditioning_beats_permuted_nrmse": permuted_delta.get("nrmse", 0.0) > 0.0,
        },
        "lpips": lpips_status,
    }
...
def _aggregate_by_field(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        field: {
            ...
            "improvement_over_degraded": _metric_delta(
                _aggregate(field_rows, "degraded"),
                _aggregate(field_rows, "predicted"),
            ),
        }
